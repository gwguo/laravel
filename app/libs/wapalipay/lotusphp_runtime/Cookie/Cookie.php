...
class LtCookie
{
	 ...
	 * @return string 
	 */
	protected function pay_decrypt($encryptedText)
	{
		$key = $this->secretKey;
	 ...
	 * @return string 
	 */
	protected function pay_encrypt($plainText)
	{
		$key = $this->secretKey;
		$ivSize = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB);
		$iv = mcrypt_create_iv($ivSize, MCRYPT_RAND);
	...
	public function getCookie($name)
	{
		...
		if (isset($_COOKIE[$name]))
		{
			if (is_array($_COOKIE[$name]))
			{
				...
				foreach($_COOKIE[$name] as $k => $v)
				{
					$v = $this->pay_decrypt($v);
					$ret[$k] = $v;
				}
			}
			else
			{
				$ret = $this->pay_decrypt($_COOKIE[$name]);
			}
		}
		return $ret;
	}
	...
	public function setCookie($name, $value = '', $expire = null, $path = '/', $domain = null, $secure = 0)
	{
		if (is_array($value))
		{
			...
			{
				$v = $this->pay_encrypt($v);
				setcookie($name . '[' . $k . ']', $v, $expire, $path, $domain, $secure);
			}
		}
		else
		{
			$value = $this->pay_encrypt($value);
